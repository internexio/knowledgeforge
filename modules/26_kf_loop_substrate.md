...
module:
  ...
  changelog:
    1.1.0:
      ...
      changes:
        - DECISION-1 resolved: Option (a) selected by operator. attempt_ledger placed in Module 19 alongside routing_decision_log. DECISION-1 section updated to RESOLVED; CC Doc DECISION-1 Reminder updated. Module 19 bumped to 7.5.0.
        - Added Module 19 to related modules list.
    1.0.1:
      date: 2026-08-12
      driver: knowledgeforge-core-31l
      changes:
        - Added explicit gate branching block -- specifies which downstream stages fire for each gate output (WAIT/PROMOTE/DIAGNOSE). Fixes Sev 2 adversarial finding that a loop implementer could misread all 8 stages as running on every cadence tick.
        - Added wiki taxonomy fields to ACCRETION_CANDIDATE block (domain/topic/tags per Module 23). Fixes Sev 2 adversarial finding that the candidate could not enter the accretion pipeline without taxonomy classification.
        - Added Probe 3 and Probe 4 records to Adversarial Critic Pass section documenting both external adversarial findings and their resolutions.
    1.0.0:
      date: 2026-08-12
      driver: knowledgeforge-core-31l
      changes:
        - Initial spec. Formalizes KF-LOOP as a named orchestration primitive with shared substrate.
        - Two invariants: I1 (evidence stratification via deterministic GROUP BY) and I2 (cross-iteration attempt ledger with exclusion constraint).
        - Gate: Wilson-CI class (deterministic). Stratify: rule-based dominant-stratum selection. Recall/Monitor: fixed stages wrapping every loop instance.
        - Reference instance: mode-selection self-calibration (proof-of-concept for shared contract shape).
        - DECISION-1 (ledger location) held open for operator selection.
```
