...
def add_compile_header(content: str, source_module: str, output_type: str,
                        version: str, *, section: str | None = None) -> str:
    """Attach a compilation metadata comment to generated files.
    ...
    Placement:
    - Header-first (doc/skill/rule): trace on line 1, before the body.
    - Frontmatter-first (agents): trace on the line AFTER the closing --- of
      the YAML block. The Claude Code agent loader silently skips files with
      any content before the opening --- (5981a7e / knowledgeforge-core-x8j),
      so line 1 must stay ---. In-file trace is still valuable for humans
      opening the compiled artifact; manifest is the machine-authoritative
      map.

    section: when provided (e.g. "CC Skill — KF Fit Check"), included in the
    header so two outputs compiled from the same module file (e.g. navigator.md
    and fit-check.md both from 01_navigator.md) remain individually traceable.
    """
    section_part = f" | section: {section}" if section else ""
    header = (
        f"<!-- Generated by kf-compile | source: {source_module}{section_part}"
        f" | type: {output_type} | version: {version} -->\n"
    )
    if content.lstrip().startswith("---"):
        # Frontmatter-first: inject trace after closing --- so line 1 stays ---.
        closing = content.find("\n---\n", 4)
        if closing == -1:
            # Malformed frontmatter (no closing ---) — return unchanged rather
            # than risk displacing the opening --- delimiter.
            return content
        end = closing + 5   # position past the closing ---\n
        return content[:end] + header + content[end:]
...
def inject_toc(compiled_content: str, raw_content: str, out_type: str) -> str:
    """Inject a section TOC into compiled content.

    Only fires for doc/skill/agent types with 2+ ## headings.
    Rules and verbatim copies are left unchanged.

    Placement:
    - Frontmatter-first (agents): TOC after the closing --- and any
      immediately-following kf-compile trace comment (Option D). Loader
      requires --- on line 1.
    - Header-first (doc/skill): TOC after the compile header comment on line 1.
    """
    ...
    # Frontmatter-first: --- must stay on line 1 (agent loader requirement).
    # Inject TOC after the frontmatter block AND any immediately-following
    # kf-compile trace comment placed by add_compile_header (Option D).
    if compiled_content.startswith("---\n"):
        closing = compiled_content.find("\n---\n", 4)
        if closing != -1:
            end = closing + 5   # includes closing ---\n
            prelude = compiled_content[:end]
            rest = compiled_content[end:]
            trace_match = re.match(
                r"^(<!-- Generated by kf-compile [^>]*-->\n)", rest
            )
            if trace_match:
                prelude += trace_match.group(1)
                rest = rest[trace_match.end():]
            return f"{prelude}\n{toc}\n\n---\n{rest}"
        # Malformed frontmatter — leave unchanged rather than displace ---.
        return compiled_content

    # Header-first: compile header comment on line 1, body follows.
...
def compile_plugin_bundle(binding: dict, output_root: Path, dry_run: bool,
    ...
    for module_num, module_spec in sorted(module_outputs.items()):
        ...
        for output_def in module_spec.get("outputs", []):
            ...
            # Insert stamp immediately after the kf-compile header (line 2) for
            # files without frontmatter.  For frontmatter-first files (agents),
            # add_compile_header returned content unchanged, so we must insert
            # the bundle stamp after the closing --- so the loader stays intact.
            if content.lstrip().startswith("---"):
                # Skip opening ---; find the closing \n---
                rest = content[3:]
                closing = rest.find("\n---")
                if closing != -1:
                    end = closing + 4  # include \n---
                    if end < len(rest) and rest[end] == "\n":
                        end += 1  # include the newline that follows ---
                    content = "---" + rest[:end] + "\n" + stamp + rest[end:]
                else:
                    content = stamp + content  # no closing ---; prepend as fallback
            else:
                # No frontmatter: kf-compile header is on line 1; stamp on line 2
